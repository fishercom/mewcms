            ...
            <div className="relative overflow-hidden">
                <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 pb-4">
                    ...
                    <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        <button type="button" className="flex items-center justify-center bg-primary-700 font-medium text-sm px-4 py-2" onClick={() => router.visit('/admin/directories/create')}>
                            <Plus/>
                            Agrgar Directorio
                        </button>                    </div>
                </div>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent className="w-56" align="end">
                                            <DropdownMenuItem asChild>
                                                <Button className="block w-full" onClick={() => router.visit(route('directories.edit', item.id))} variant="ghost">
                                                    Edit
                                                </Button>
                                            </DropdownMenuItem>                                            <DropdownMenuItem asChild>
                                                <Button className="block w-full" onClick={()=>deleteDirectoryHandler(item.id)} variant="ghost">
                                                    Delete
                                                </Button>
                                            </DropdownMenuItem>
                                        </DropdownMenuContent>
